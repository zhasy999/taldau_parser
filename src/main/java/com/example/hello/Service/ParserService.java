...
public class ParserService {
    ...
    public String getUrl(String a, String b) {
        return "https://taldau.stat.gov.kz/ru/Api/GetIndexData/" + a + "?period=7&dics=" + b;
    }
            ...
            for (ArrayList<Integer> r : regions) {
                if (f.getTerms().get(0).equals(r.get(0)) && f.getTerms().get(1).equals(r.get(1))) {
                    ...
                    for (DataFormatter.Periods p : periodList) {
                        if (!p.getValue().equalsIgnoreCase("x") && dateFormat.parse(p.getDate()).equals(dateFormat.parse(dateFormat.format(today)))) {
